...
{
    /**
     * Menambahkan nilai 'pembelian_bbm' ke kolom `sumber` di tabel jurnal_umum,
     * supaya jurnal otomatis dari pembelian BBM bisa dibedakan dari sumber lain.
     * Pola identik dengan migration 2026_07_23_100002 (penambahan 'payroll') -
     * lihat migration itu untuk penjelasan driver-aware lengkap.
     */
    public function up(): void
    {
        ...
        if (! in_array('pembelian_bbm', $nilai, true)) {
            $nilai[] = 'pembelian_bbm';
        }
    ...
    public function down(): void
    {
        ...
        $masihDipakai = DB::table('jurnal_umum')->where('sumber', 'pembelian_bbm')->exists();
        if ($masihDipakai) {
            return;
        }

        preg_match_all("/'((?:[^'\\\\]|\\\\.)*)'/", $col->Type, $m);
        $nilai = collect($m[1] ?? [])->reject(fn ($v) => $v === 'pembelian_bbm')->values();
